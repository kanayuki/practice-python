...
for val in it:
    ...
    len = (height**2 + (row*carrier_distance/2) **
           2 + (col*carrier_distance/2)**2)**0.5
    s = (height-connector_length) / len
...
for i, abaqus_step in enumerate(abaqus_step_list):
    ...
    for val in it:
        ...
        match i % 4+1:
            case 1:
                # j 在第一列或最后一列，不移动，偶数时正向移动，奇数时反向移动
                # s=ceil((j+1)/row)
                # s = 0 if s == 1 or s == row else 1 if j % row % 2 == 0 else -1
    ...
    match i % 4+1:
        case 1:
            yarn_matrix[:, 1:-1:2] = np.roll(yarn_matrix[:, 1:-1:2], 1, axis=0)
            yarn_matrix[:, 2:-
                        1:2] = np.roll(yarn_matrix[:, 2:-1:2], -1, axis=0)
        case 2:
            yarn_matrix[1:-1:2,
                        :] = np.roll(yarn_matrix[1:-1:2, :], -1, axis=1)
            yarn_matrix[2:-1:2,
                        :] = np.roll(yarn_matrix[2:-1:2, :], 1, axis=1)
        case 3:
            yarn_matrix[:, 1:-
                        1:2] = np.roll(yarn_matrix[:, 1:-1:2], -1, axis=0)
            yarn_matrix[:, 2:-1:2] = np.roll(yarn_matrix[:, 2:-1:2], 1, axis=0)
        case 4:
            yarn_matrix[1:-1:2,
                        :] = np.roll(yarn_matrix[1:-1:2, :], 1, axis=1)
            yarn_matrix[2:-1:2,
